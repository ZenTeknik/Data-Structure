 ...
 // but, array have an issue with a size  on the other  hand in link list we dont have an issue with memory we have enough memory 
...
// linked list is a data structure in which elements  are explicitly ordered , that is each item conatains within itself the address of next item 

//  Node is mixture of somethings 
...
// here *next is the self refrential pointer (without pointer the compiler gives an error  bcz, we are creating object  while the defination is not completed)
