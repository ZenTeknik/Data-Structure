...
// at the end K-smallest function is define LAB 8 Extra task
#include <iostream>
using namespace std;
...
// Helper function for ksmallest
void findKthSmallest(node *root, int &k, int &count, bool &found) {
    if (root == nullptr || found) return;

    findKthSmallest(root->left, k, count, found);

    if (++count == k) {
        cout << "The " << k << "-th smallest element is: " << root->info << endl;
        found = true;
        return;
    }

    findKthSmallest(root->right, k, count, found);
}

// Function for finding the k-th smallest element
void BST::ksmallest(node *root, int k) {
    int count = 0;
    bool found = false;
    findKthSmallest(root, k, count, found);
